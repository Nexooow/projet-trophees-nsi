...
class ExpeditionState(State):
    ...
    def __init__(self, state_manager):
        ...
        self.perlin = Perlin(scale=30, octaves=4, steps=4, normalize=True)
        ...
        self.cells_color = {
            col: [self.draw_cells(color, mask) for mask in range(16)]
    ...
    def draw_map_state(self):
        ...
        for cx, cy in list(self.chunks):
            if (
                abs(cx - start_cx) > MAX_CHUNK_DIST
    ...
    def draw_node_menu(self):
        ...
        self.screen.blit(
            font.render("Manual Battle", True, (255, 255, 255)),
            (manual_rect.x + 10, manual_rect.y + 5),
        )
        self.screen.blit(
            font.render("Auto Resolve", True, (255, 255, 255)),
            (auto_rect.x + 10, auto_rect.y + 5),
        )
        self.menu_rects = {"manual": manual_rect, "auto": auto_rect}
    ...
    def generate_chunk(self, cx, cy):
        surf = pygame.Surface((CHUNK_SIZE * CELL_SIZE, CHUNK_SIZE * CELL_SIZE))
        weights = {
        ...
        for x in range(CHUNK_SIZE):
            for y in range(CHUNK_SIZE):
                ...
                if weights[(x - 1, y)] == w:
                    mask |= 8
                surf.blit(self.cells_color[w][mask], (x * CELL_SIZE, y * CELL_SIZE))
        return surf
