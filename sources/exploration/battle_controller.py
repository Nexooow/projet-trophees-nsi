import pygame
from lib.pathfinding import closest_enemy, neighbors, reachable_tiles_nx, shortest_path
    ...
    def handle_player_turn(self, events):
        ...
        enemies_present = any(
            (
                u.team != active.team
                and (
                    ((u.x, u.y) == (active.x, active.y))
                    or (u.x, u.y)
                    in neighbors(active.x, active.y, grid.width, grid.height)
                )
            )
            for u in units
        )
