...
class BattleRenderer:
    def __init__(self, model, screen, sidebar):
        ...
        self.game_surface = pygame.Surface((screen.get_width() - 250, screen.get_height()))
    ...
    def draw_grid(self):
        ...
        for (x, y) in tiles.keys():
            pygame.draw.rect(self.game_surface, (255, 255, 0),
                             pygame.Rect(x * grid.tile, y * grid.tile, grid.tile, grid.tile), 2)
            ...
            if mouse_over(unit) and unit != self.model.active_unit:
                ...
                for (x, y) in tiles.keys():
                    pygame.draw.rect(self.game_surface, (255, 0, 0),
                                     pygame.Rect(x * 50, y * 50, 50, 50), 2)

    def draw_sidebar(self):
        self.sidebar.draw(self.ui_surface, self.model.units, self.model.active_unit, None)